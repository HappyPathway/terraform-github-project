...
run "verify_workspace_file_creation" {
  ...
  assert {
    condition = module.repository_files.files["test-workspace.code-workspace"] != null
    error_message = "Workspace file should be created in base repository"
  }

  assert {
    condition = contains(
      jsondecode(module.repository_files.files["test-workspace.code-workspace"].content).extensions.recommendations,
      "github.copilot"
    )
